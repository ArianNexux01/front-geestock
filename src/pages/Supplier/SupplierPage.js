...
export default function SupplierPage() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        if (search.length <= 1) {
          const url = `/supplier?onlyActive=${userData.data.position !== "3" ? "0" : "1"}`;
  ...
  const handleSearch = async () => {
    try {
      ...
      const url = `/supplier?searchParam=${search}&onlyActive=${userData.data.position !== "3" ? "0" : "1"}`;
      const response = await api.get(url)
      setData(response.data)
      console.log(response.data)
    ...
    <>
      ...
      <Container>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mt={3} mb={5}>
          ...
          <Button variant="contained" onClick={() => { navigate("/dashboard/fornecedor/cadastrar") }} startIcon={<Iconify icon="eva:plus-fill" />}>
