...
export default function RequestsPage() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        if (search.length <= 1) {
          const url = `/request/by-state/warehouseoutcomming/${requestStatus.value}/${curentWarehouse?.id ?? curentWarehouse}`;
          const response = await api.get(url)
          setData(response.data)
        }
      } catch (e) {
  ...
  const handleSearch = async () => {
    try {
      ...
      const url = `/ request / warehouseoutcomming ? searchParam = ${search}& onlyActive=1`;
      const response = await api.get(url)
      setData(response.data)
      ...
      <Container>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mt={3} mb={5}>
          ...
          <Box sx={{ width: "20%", display: "flex", justifyContent: "space-between" }}>
            {userData.data.position === "3" && <><Button variant="contained" onClick={() => { navigate("/dashboard/requisicao/minhas") }} startIcon={<Iconify icon="eva:eye-fill" />}>
              Requisições Para mim
            </Button></>}
            {userData.data.position !== "3" && <> <Button variant="contained" onClick={() => { navigate("/dashboard/requisicao/cadastrar") }} startIcon={<Iconify icon="eva:plus-fill" />}>
              Criar Requisição
            </Button></>}
          </Box>
        </Stack>
