...
export default function PiecesPage() {
  ...
  const handleDownloadFiles = async () => {
    ...
    if (res.status === 200) {
      window.open(process.env.REACT_APP_API_URL + "/piece-data.xlsx")
    }
  }
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        if (search.length <= 1) {
          let url = ""
          if (curentWarehouse !== "Todos") {

            url = Number(userData.data?.position) >= 1 ? `/piece/warehouse/${curentWarehouse}?onlyActive=${userData.data.position === "1" ? "0" : "1"}` : `/piece?onlyActive=${userData.data.position === "1" ? "0" : "1"}`;
          } else {
            url = `/piece?onlyActive=${userData.data.position === "1" ? "0" : "1"}`
          }
          console.log("url: ", url)
          const response = await api.get(url)
          setData(response.data)
      ...
      <Container>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mt={3} mb={5}>
          ...
          <Box>
            ...
            {userData.data.position !== "3" && <Button variant="contained" onClick={() => { navigate("/dashboard/peca/cadastrar") }} startIcon={<Iconify icon="eva:plus-fill" />}>
              Cadastrar Peça
            </Button>}
            {userData.data.position === "1" && <Button style={{ marginLeft: '10px' }} variant="contained" onClick={handleOpen} startIcon={<Iconify icon="eva:upload-fill" />}>
              Carregar Peças
            </Button>}
          </Box>
        </Stack>
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">
                          ...
                          <Label color={isActive ? 'success' : 'error'}>{!isActive ? 'Inactivo' : 'Activo'}</Label>

                        </TableCell>
                        <TableCell align="right">
                          <IconButton size="large" color="inherit" onClick={(e) => { handleOpenMenu(e, userData.data.position !== "3" ? id : Piece.id, isActive, id, locationInWarehouse) }}>
        ...
        >
          <MenuItem onClick={() => { navigate(`/dashboard/peca/editar/${actualId}?pieceWarehouseId=${pieceWarehouseId}&locationInWarehouse=${locationInWarehouseData}`) }}>
            ...
            {userData.data.position !== "3" ? 'Editar' : 'Visualizar'}
          </MenuItem>

          {userData.data.position !== "3" && <MenuItem onClick={() => { handleChangeStatus() }} >
            <Iconify icon={'eva:trash-2-outline'} sx={{ mr: 2 }} />
            {!status ? 'Activar' : 'Desactivar'}
          </MenuItem>}
        </Popover>
        {userData.data.position === "1" && <Button style={{ marginTop: '30px' }} variant="contained" onClick={() => { handleDownloadModel() }} startIcon={<Iconify icon="eva:download-fill" />}>
          Baixar Template
        </Button>}
