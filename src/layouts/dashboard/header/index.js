...
export default function Header({ onOpenNav }) {
  ...
  const getData = async () => {
    if (userData.data.position === "3") {
      setCurentWarehouse(userData.data.warehouse[0]?.id)
    } else {
      const responseWarehouse = await api.get("/warehouse?onlyActive=1")
      setDataWarehouse(responseWarehouse.data.filter(piece => piece.type === "Armazém"))
      console.log("CONSOLE LOG: ", responseWarehouse.data)
    ...
    <StyledRoot>
      <StyledToolbar>
        ...
        >
          ...
          {userData.data?.position !== "3" && <NotificationsPopover />}
          <AccountPopover />
        </Stack>
